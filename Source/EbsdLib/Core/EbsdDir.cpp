...
#define FILE_INFO_CLASS_NAME EbsdDir
...
//
// -----------------------------------------------------------------------------
std::vector<std::string> FILE_INFO_CLASS_NAME::entryList(const std::string& path)
{
  DIR* dir = NULL;
...
//
// -----------------------------------------------------------------------------
bool FILE_INFO_CLASS_NAME::mkdir(const std::string& name, bool createParentDirectories)
{
#if defined (WIN32)
  std::string dirName = name;
  if (createParentDirectories) {
    dirName = FILE_INFO_CLASS_NAME::toNativeSeparators(FILE_INFO_CLASS_NAME::cleanPath(dirName));
    ...
    if(dirName[0] == '\\' && dirName[1] == '\\')
    {
      ...
      {
        if(dirName[i] != FILE_INFO_CLASS_NAME::Separator)
        {
          oldslash = i;
          break;
        }
      ...
      {
        oldslash = dirName.find(FILE_INFO_CLASS_NAME::Separator, oldslash);
      }
      }
      for (std::string::size_type slash=0; slash != std::string::npos; oldslash = slash) {
        slash = dirName.find(FILE_INFO_CLASS_NAME::Separator, oldslash + 1);
  ...
  if (createParentDirectories)
  {
    dirName = FILE_INFO_CLASS_NAME::cleanPath(dirName);
...
//
// -----------------------------------------------------------------------------
bool FILE_INFO_CLASS_NAME::rmdir(const std::string& name, bool recurseParentDirectories)
{
#if defined (WIN32)
  std::string dirName = name;
  if (recurseParentDirectories)
  {
    ...
    for(std::string::size_type oldslash = 0, slash = dirName.length(); slash > 0; oldslash = slash)
    {
      ...
      if(!_rmdir(chunk.c_str()))
        return oldslash != 0;
      slash = dirName.find_last_of(FILE_INFO_CLASS_NAME::Separator, oldslash - 1);
      }
      return true;
  }
...
//
// -----------------------------------------------------------------------------
bool FILE_INFO_CLASS_NAME::remove(const std::string& fsPath)
{
  return UNLINK(FILE_INFO_CLASS_NAME::toNativeSeparators(fsPath).c_str()) == 0;
}
