...
% plus the 3 Miller-Bravais / Miller indices to plot. The hex/trig classes
% pin the alignment EXPLICITLY to 'X||a*' so this side-by-side stays
% apples-to-apples with PoleFigureLaueComparisonTest, which renders X||a*.
% (Do not rely on the MTEX default alignment — it is not guaranteed X||a*.)
% Note: EbsdLib's public default convention is X||a (TSL) as of 3.1; this
% diagnostic deliberately compares the X||a* path that the C++ test pins.
...
laueMap('23')  = struct('cs', crystalSymmetry('m-3'), ...
    'h', {{[0 0 1], [0 1 1], [1 1 1]}});
% hexagonal
laueMap('622') = struct('cs', crystalSymmetry('6/mmm', [1 1 1.6], 'X||a*'), ...
    'h', {{[0 0 0 1], [1 0 -1 0], [2 -1 -1 0]}});
laueMap('6')   = struct('cs', crystalSymmetry('6/m', [1 1 1.6], 'X||a*'), ...
...
laueMap('4')   = struct('cs', crystalSymmetry('4/m'), ...
    'h', {{[0 0 1], [1 0 0], [1 1 0]}});
% trigonal
laueMap('32')  = struct('cs', crystalSymmetry('-3m', [1 1 1.6], 'X||a*'), ...
    'h', {{[0 0 0 1], [0 -1 1 0], [1 -1 0 0]}});
laueMap('3')   = struct('cs', crystalSymmetry('-3', [1 1 1.6], 'X||a*'), ...
