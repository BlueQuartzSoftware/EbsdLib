...
| `mtex_pole_figure_positions.m`          | MATLAB script. Generates **both** golden CSVs from MTEX, one per hex/trig Cartesian convention (a single run writes both files). |
| `run_mtex_pole_figure_positions.sh`     | zsh wrapper that launches MATLAB headlessly with MTEX initialized. |
| `mtex_pole_figure_positions.csv`        | **The committed `X\|\|a*` (MTEX/Oxford) golden.** Loaded by `PoleFigurePositionTest` at test time and always validated. Regenerated by the MATLAB script above; the only reason to regenerate is if the canonical orientation list, Laue dispatch, or plane-family table changes. |
| `mtex_pole_figure_positions_xa.csv`     | **The committed `X\|\|a` (TSL/EDAX) golden** — the 3.1 public default convention. Loaded and validated by `PoleFigurePositionTest` *when present*; if absent the test skips the `X\|\|a` pass with a loud warning rather than failing. Produced by the same MATLAB run as the `X\|\|a*` golden. |
...
the script's `laue` struct. The family **labels are convention-independent**
(brace notation, identical in both goldens); only the hex/trig pole
**positions** differ between the `X||a*` and `X||a` goldens (a 30 deg basis
rotation about c). Every non-hex/trig class produces identical rows in both
files.
...
You only need to regenerate the goldens if the canonical orientation list,
the Laue dispatch, or the plane-family table changes — i.e. if the schema of
what's being validated changes. Routine EbsdLib code changes do not need a
golden regeneration.

A single MATLAB run writes **both** goldens (`mtex_pole_figure_positions.csv`
for `X||a*` and `mtex_pole_figure_positions_xa.csv` for `X||a`). The `X||a`
golden is required for `PoleFigurePositionTest` to actually validate the 3.1
default convention — until it is committed, the test skips the `X||a` pass
with a warning.
...
`mtex_pole_figure_positions.m`, which writes both
`mtex_pole_figure_positions.csv` and `mtex_pole_figure_positions_xa.csv`
into this same directory. After regeneration, run the test:
