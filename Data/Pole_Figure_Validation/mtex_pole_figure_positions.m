...
%   plane_family, x, y
%
% Comparison is then a per-bucket nearest-neighbor match between the golden
% CSV and ebsdlib_pole_figure_positions.csv.
%
% TWO goldens are emitted, one per hexagonal/trigonal Cartesian convention:
%   mtex_pole_figure_positions.csv     -> X||a* (MTEX/Oxford)
%   mtex_pole_figure_positions_xa.csv  -> X||a  (TSL/EDAX, the 3.1 default)
% The convention only changes the hex/trig crystalSymmetry alignment (a 30
% deg basis rotation about c). Every other Laue class ignores it, so those
% rows are identical between the two files — exactly mirroring EbsdLib's
% getDefaultPoleFigureNames()/generateSphereCoordsFromEulers() behavior.
% The plane-family labels are convention-independent (brace notation) and
% MUST match the C++ getDefaultPoleFigureNames() output verbatim.
%
% Usage:
...
%      automatically.
%   2. Both CSVs are written into the same directory as this script
%      (Data/Pole_Figure_Validation/). They are the committed goldens
%      against which PoleFigurePositionTest.cpp compares the EbsdLib output.
...
% -----------------------------------------------------------------------------
% Conventions to emit. The hexagonal/trigonal crystalSymmetry alignment is
% the only thing that changes per convention; all other Laue classes ignore
% it and produce identical rows in both files.
% -----------------------------------------------------------------------------
conventions = { ...
    'X||a*', fullfile(scriptDir, 'mtex_pole_figure_positions.csv')    ; ...
    'X||a',  fullfile(scriptDir, 'mtex_pole_figure_positions_xa.csv') ; ...
};

ss = specimenSymmetry('1');

for ci = 1:size(conventions, 1)
    xAlign  = conventions{ci, 1};
    csvPath = conventions{ci, 2};
    ...
    % cs : MTEX crystalSymmetry instance (hex/trig use the current alignment)
    % h : 1x3 cell array of Miller index tuples (3- or 4-element)
    % labels : 1x3 cell array of pole figure label strings (must match
    %          EbsdLib output verbatim; convention-independent brace notation)
    % -------------------------------------------------------------------------
    laue = struct([]);
...
fprintf('These are the committed goldens -- PoleFigurePositionTest.cpp loads\n');
fprintf('mtex_pole_figure_positions.csv (X||a*) always, and\n');
fprintf('mtex_pole_figure_positions_xa.csv (X||a) when present. See ReadMe.md.\n');
